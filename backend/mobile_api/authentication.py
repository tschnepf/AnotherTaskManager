...
def _build_realm_url(raw_base_url: str, realm: str) -> str:
    return build_realm_url(raw_base_url, realm)
...
def _provision_identity_from_claims(*, issuer: str, subject: str, payload: dict[str, Any]) -> OIDCIdentity | None:
    return resolve_or_provision_identity(
        issuer=issuer,
        subject=subject,
        claims=payload,
        auto_provision_users=_auto_provision_users_enabled(),
        auto_provision_organization=_auto_provision_organization_enabled(),
    )
