# Task Hub iOS + Mobile API Implementation Plan (Self-Hosted Keycloak, Agent-Executable)

Execution tracks split for direct use:
1. Server/backend/infra: `Spec/ios-server-implementation-plan.md`
2. Xcode/iOS app: `Spec/ios-xcode-app-implementation-plan.md`
...
3. Task deletions emit tombstones.
4. Event emission uses `transaction.on_commit`.
5. Event retention purge is scheduled and tested.
...
### Track B - iOS App Build (execute in Xcode project)
1. Perform iOS app implementation in Section `10` only.
2. Execute all `Step I0` through `Step I5` in Xcode (SwiftUI app, widget, intents, notification extension, iOS tests).
3. Use the prompt blocks under each iOS step directly with your Xcode AI agent.

### Track C - Cross-track validation
...
7. `GET /api/mobile/v1/sync/delta?cursor=<n>&limit=<n>`
...
3. Define OIDC + API scopes:
   ...
   - `mobile.sync`
   - `mobile.notify`
4. Define API audience and mapper.
5. Make setup idempotent for redeploys.
6. Add deterministic secret-hygiene validator for realm exports.
...
Tasks:
1. Add `NotificationPreference` with IANA timezone validation.
2. Define precedence rules: user preference overrides org default.
3. Add DST-sensitive quiet-hours tests.
4. Add `MobileDevice` register/update/delete endpoints and ownership/org checks.
5. Implement device upsert semantics keyed by token hash/environment or installation ID.
...
4. Document key rotation and incident handling runbook.

Validation:
```bash
. .venv/bin/activate
pytest -q backend/mobile_api/tests/test_mobile_security_guards.py
```
...
## 10. iOS Scaffold Plan (Execute in Xcode / iOS Project)
...
Xcode AI prompt:
```text
Create iOS SwiftUI app TaskHubMobile (iOS 17+).
Add targets:
- TaskHubMobile
...
Handle onboarding_required response by showing onboarding instruction UI.
```
...
```text
Implement:
1) APNs registration + /devices endpoints
2) preferences UI for /notifications/preferences
3) CreateTaskIntent calling /intents/create-task with Idempotency-Key
4) Widget snapshot cache from /widget/snapshot in App Group
```
